...
void MyFrame::OnSearch(wxCommandEvent&) {
    ...
    for (size_t i = 0; i < fileCache.size(); ++i) {
        ...
        // -------------------------------------------------------
        // MODE 0 — Full line
        // Columns: each comma/space-separated token from the line.
        // We preserve the original values (before comma-stripping)
        // but split on comma OR whitespace for column boundaries.
        // -------------------------------------------------------
        if (lastMode == 0) {
            display = std::to_string(i + 1) + " | " + line;

            // Split original line by comma first, then trim spaces
            // so  "SR_SOUDER SCS1D009543_L2H2,V_rapide,1,1,Tool_Pince1,wobj_XDF;"
            // becomes 6 neat columns after the line-number column.
            std::vector<std::string> parts = splitByDelim(line, ',');
            for (auto& p : parts) {
                // Each part may still have leading/trailing spaces
