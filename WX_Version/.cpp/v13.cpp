    ...
    wxDefaultPosition, wxSize(1000, 650))
{
    ...
    // Placeholder tab — evita área preta quando não há abas
    {
...
wxTextCtrl* MyFrame::CreateSnapshotTab(const std::string& title) {
    ...
    // Executa APÓS o ciclo de eventos terminar — elimina o flash preto
    CallAfter([page, ctrl]() {
        page->Layout();
        page->Refresh();
        page->Update();
...
void MyFrame::OnSearch(wxCommandEvent&) {
    ...
    // Remove a aba placeholder "..." se ainda estiver presente
