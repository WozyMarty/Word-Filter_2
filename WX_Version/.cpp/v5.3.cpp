...
private:
    ...
    wxTimer searchTimer; // 🔥 debounce timer
...
public:
    MyFrame()
        : wxFrame(nullptr, wxID_ANY, "Debounced Threaded Search", wxDefaultPosition, wxSize(850, 600)),
          searchTimer(this) {
    ...
    // ---------- 🔥 typing = restart timer ----------
    void OnTyping(wxCommandEvent&) {
        searchTimer.Stop();
        searchTimer.Start(1000, wxTIMER_ONE_SHOT); // 2 seconds delay
    }
    ...
    void StartSearch() {
        ...
        if (file.empty() || word.empty()) return;
