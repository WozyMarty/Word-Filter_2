...
bool parseJointtarget(const std::string& line, ParsedTarget& out) {
    if (line.find("jointtarget") == std::string::npos)
        return false;

    out.type = TargetType::JOINTTARGET;

    size_t start = line.find("jointtarget") + 11;
    while (start < line.size() && line[start] == ' ') start++;
    ...
    while (std::getline(iss, value, ',')) {
        out.joints.push_back(trim(value));
    }
...
bool parseTarget(const std::string& line, ParsedTarget& out) {
    if (parseRobtarget(line, out)) return true;
    if (parseJointtarget(line, out)) return true;
    return false;
}
...
public:
    MyFrame() : wxFrame(nullptr, wxID_ANY,
        ...
        wxString choices[] = { "Full line", "First 2 words", "Parsed targets" };
        ...
        worker = std::thread([=]() {
            ...
            for (const auto& lineOriginal : fileCache) {
                ...
                if (choice == 0) {
                    ...
                    if (pt.type == TargetType::ROBTARGET) {
                        row.type = "robtarget";
                        row.name = pt.name;
                        row.x = pt.x;
                        row.y = pt.y;
                        ...
                        display = "robtarget; " + row.name +
                                  "; X=" + row.x +
                                  " Y=" + row.y +
                                  " Z=" + row.z;
                    }
                    else if (pt.type == TargetType::JOINTTARGET) {
                        row.type = "jointtarget";
                        row.name = pt.name;
                        row.joints = pt.joints;

                        display = "jointtarget; " + row.name + "; ";
                        for (size_t i = 0; i < row.joints.size(); ++i)
                            display += "J" + std::to_string(i + 1) + "=" + row.joints[i] + " ";
                    }
                }
