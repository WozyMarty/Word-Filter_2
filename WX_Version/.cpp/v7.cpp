...
bool parseTarget(const std::string& line, ParsedTarget& out) {
    return parseRobtarget(line, out);
}
...
public:
    MyFrame() : wxFrame(nullptr, wxID_ANY,
        "SIM ROBOTICS Edition - Snapshot Search Tool",
        ...
        main->Add(notebook, 1, wxEXPAND | wxALL, 10);
    ...
    wxTextCtrl* CreateSnapshotTab(const std::string& title) {
        ...
        notebook->AddPage(page, title, true);
    ...
    void StartSearch() {
        ...
        wxTextCtrl* currentOutput = CreateSnapshotTab(word);
        ...
        worker = std::thread([=]() mutable {
            ...
            for (const auto& lineOriginal : fileCache) {
                ...
                if (choice == 0) {
                    ...
                    display = std::to_string(lineNumber) +
                        " | robtarget " + row.name +
                        " X=" + row.x +
                        " Y=" + row.y +
                        " Z=" + row.z;
                ...
                if (buffer.size() >= 100) {
                    ...
                    wxTheApp->CallAfter([currentOutput, copy]() {
                        for (auto& l : copy)
                            currentOutput->AppendText(l + "\n");
                    });
                }
            ...
            wxTheApp->CallAfter([this, currentOutput, local, hits]() {
                results = local;
                currentOutput->AppendText("\nTotal: " + std::to_string(hits) + "\n");
